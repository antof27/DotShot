...
def objective(trial):
    ...
    mean_reward, std_reward = evaluate_policy(model, game_env, n_eval_episodes=7, deterministic=True)
...
def main():
    parser = argparse.ArgumentParser(description="Tune PPO hyperparameters using Optuna")
    ...
    print(f"Starting Optuna PPO hyperparameter tuning ({args.trials} trials)...")
